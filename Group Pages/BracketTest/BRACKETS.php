...
if (isset($_POST['submit'])) {
  ...
  if ($stmt->rowCount() == 0) {
    ...
    if ($row['PIN'] != $PIN) {
      ...
      // Pin matches, update selections
      echo "Pin matches, update selections.";
...
elseif (isset($_POST['retrieve'])) {
  ...
  if ($stmt->rowCount() == 0) {
    ...
    echo "Invalid user or PIN.";
  } else {
    // User and PIN match, retrieve selections
    $stmt = $conn->prepare("SELECT selections FROM userbracket WHERE name = ?");
    $stmt->execute([$name]);

    if ($stmt->rowCount() == 0) {
      // No selections found for user, return error
      echo "No selections found for user.";
    } else {
      // Selections found, populate form
      $row = $stmt->fetch();
      $selections = $row['selections'];

      // Populate the form with the selections
      // ...
    }
  }
}
